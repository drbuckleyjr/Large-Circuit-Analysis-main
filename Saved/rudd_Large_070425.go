// rudd_Large_070425.go
// version 4 JUL 2025 @ 2340 hrs
...
func main() {
	...
	peek := func(fp int, s nd, fault_A, fault_C string) {
		...
		// need to modify peek() to return the list of S/I's that it has
		// displayed. This will allow the user to select one of the S/I's
		// and continue the simulation with that S/I.
	...
	foo := func() {
		...
		for { // outer continuous loop peek
			...
			var s string = "s0" // activation state

			for { // inner continuous loop peek

				// ---- peek() -----------------------------------------------
				peek(fp, str2nd(s), fault_A, fault_C)
				// -----------------------------------------------------------

				// it's okay to allow peek() to continue to display the list.
				...
				// this is where we extract a new fp, s; insert new step here
				// use the user entered S/I to extract fp and s from a table
				// that peek() has returned. So, fp and s have new values.
				// modify peek() to return the list of S/I's.
