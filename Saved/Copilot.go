...
var eq func(rudd.Node, rudd.Node) rudd.Node               // BDD equality operation
var isTrue func(rudd.Node) bool                           // Check if node represents TRUE
var isFalse func(rudd.Node) bool                          // Check if node represents FALSE
var isNull func(rudd.Node) bool                           // Check if node is NULL/invalid
var toBool func(rudd.Node) bool
var null rudd.Node
...
func init() {
	...
	and = bdd.And
	or = bdd.Or
	eq = func(n1, n2 rudd.Node) rudd.Node {
		// Since bdd.Equal returns bool, we need a wrapper that returns a Node
		if bdd.Equal(n1, n2) {
			return bdd.True()
		}
		return bdd.False()
	...
	// Helper functions to convert BDD nodes to boolean values
	isTrue = func(n rudd.Node) bool {
		return bdd.Equal(n, bdd.True())
	}
	isFalse = func(n rudd.Node) bool {
		return bdd.Equal(n, bdd.False())
	}
	isNull = func(n rudd.Node) bool {
		return bdd.Equal(n, null)
	}
	toBool = func(n rudd.Node) bool {
		return !bdd.Equal(n, bdd.False())
	}
	null = bdd.False()
...
// xx1: Accumulate fault-free simulation results for each S/I
...
// xx2: Accumulate fault_A simulation results for each S/I
...
func simulateAndCategorizeFaults(faultList []string, phaseName string) {
	...
	// Reset globals before fault-free simulation
	first, ns16h, ns8h, ns4h, ns2h, ns1h = true, false, false, false, false, false
	for _, si := range accumulatedSIs {
		faultFreeResults = append(faultFreeResults, simulateStateInputWithFault(si, ""))
	}

	// Reset globals before fault_A simulation
	first, ns16h, ns8h, ns4h, ns2h, ns1h = true, false, false, false, false, false
	for _, si := range accumulatedSIs {
		faultAResults = append(faultAResults, simulateStateInputWithFault(si, originalFaultA))
	}
	...
	// For XX4, offer detailed results with pagination
	fmt.Print("\nShow detailed simulation results? (y/n): ")
	var showDetails string
	fmt.Scanln(&showDetails)
...
// simulateCircuitLogic simulates the circuit from present state to next state
...
func simulateCircuitLogic(ps16_i, ps8_i, ps4_i, ps2_i, ps1_i nd,
	...
	f1_s, f1_1 := and2Rule(s12_s, i5_s, s12_1, i5_1) // f1 is not within next-state line cone
	f1_s = applyFault(f1_s, f1_1, fault_A, "f1")

	f2_s, f2_1 := and2Rule(s27_s, i4_s, s27_1, i4_1) // f2 is not within next-state line cone
	f2_s = applyFault(f2_s, f2_1, fault_A, "f2")

	f3_s, f3_1 := and2Rule(s15_s, i0_s, s15_1, i0_1) // f3 is not within next-state line cone
	f3_s = applyFault(f3_s, f3_1, fault_A, "f3")

	f4_s, f4_1 := and2Rule(s27_s, i2_s, s27_1, i2_1) // f4 is not within next-state line cone
	f4_s = applyFault(f4_s, f4_1, fault_A, "f4")

	f5_s, f5_1 := and2Rule(s0_s, i7_s, s0_1, i7_1) // f5 is not within next-state line cone
	f5_s = applyFault(f5_s, f5_1, fault_A, "f5")

	f6_s, f6_1 := and2Rule(s27_s, i1_s, s27_1, i1_1) // f6 is not within next-state line cone
	...
	out4_s, out4_1 := or2Rule(f1_s, f2_s, f1_1, f2_1) // out4 is not within next-state line cone
	out4_s = applyFault(out4_s, out4_1, fault_A, "out4")

	out2_s, out2_1 := or2Rule(f3_s, f4_s, f3_1, f4_1) // out2 is not within next-state line cone
	out2_s = applyFault(out2_s, out2_1, fault_A, "out2")

	out1_s, out1_1 := or2Rule(f5_s, f6_s, f5_1, f6_1) // out1 is not within next-state line cone
...
} // end of generateFaultPatterns() ====================================================

// displayAvailableTransitions shows all possible S/I transitions from current state
// Displays reachable states and populates the S/I mapping for user selection
func displayAvailableTransitions(fp int, s nd, fault_A string, fault_C string,
	...
	out4_s, out2_s, out1_s, ns16_s, ns8_s, ns4_s, ns2_s, ns1_s :=
		simulateCircuitLogic(pt16, pt8, pt4, pt2, pt1, fault_A)
	// =============================================================
...
} // end of displayAvailableTransitions() =================================================
...
// End of simulateSingleTimeframe() =========================================
...
func getAllPossibleFaults() []string {
	...
	// --- REQUIRES ALL FAULTS FROM simulateCircuitLogic() ---   <<< ======================
	signals := []string{
		//--- level 0 ---
	...
	for {
		...
		if fault_C == "" {
			...
			for {
				...
				fmt.Print("\nEnter selected S/I (example: s12i5) or control (999/xx0/xx1/xx2/xx3/xx4): ")
				var input string
				fmt.Scanln(&input)
				...
				default:
					...
					if strings.HasPrefix(input, "s") && strings.Contains(input, "i") {
						...
						fmt.Println("Invalid input. Example S/I format (s12i5) or control codes (999/xx0/xx1/xx2/xx3/xx4)")
					}
