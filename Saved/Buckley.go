...
func main() {
	...
	allSAT := func(f nd, str2nd func(string) nd) []string {
		...
		return g // Return a list of S/I's in the nd function
	}
	...
	// ps2ns ===========================================================
	// =================================================================
	ps2ns := func(ps16_i, ps8_i, ps4_i, ps2_i, ps1_i nd, fault_A string)
	...
	ns2fp := func(out4_s, out2_s, out1_s, ns16_s, ns8_s, ns4_s, ns2_s, ns1_s nd)
	...
	// -- setUP and one_set_BOOL simulate a fault_C circuit --
	...
	// --- peek() ---
	peek := func(fp int, s nd, fault_A string,
		allSATFunc func(nd, func(string) nd) []string,
		nd2strFunc func(nd) string) map[string]SIMapping {
		...
		//fmt.Println("-------------------------------------------------")
		//fmt.Println("circuit: Large,  fault_A: ", fault_A)
		...
		dispdM := func(dMfpx S, lns nd, fpn int,
			allSATFunc func(f nd, str2nd func(string) nd) []string,
			...
			return siMap // Return the updated S/I mapping
		}
		...
		return siMap // Return the S/I mapping for further use
	...
	foo := func() {
		...
		for { // outer continuous loop peek
			...
			var snd nd = s0                     // initial activation <fp, ns>
			...
			for { // inner continuous loop peek
				...
				// ---- peek() -----------------------------------------------
				siMap = peek(fp, snd, fault_A, fault_C, allSAT, nd2str)
				// -----------------------------------------------------------
				...
				if mapping, exists := siMap[usi]; exists {
					fp = mapping.fp
