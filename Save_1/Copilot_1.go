...
func main() {
	...
	ns2fp := func(out4_s, out2_s, out1_s, ns16_s, ns8_s, ns4_s, ns2_s,
		...
		fp0 := and(not(ns16_s), not(ns8_s), not(ns4_s), not(ns2_s), not(ns1_s))
		...
		dMfp0 := decompMap(fp0, out4_s, out2_s, out1_s, collectedSIs)
		...
		return dMfp0, dMfp1, dMfp2, dMfp3, dMfp4, dMfp5, dMfp6, dMfp7, dMfp8, dMfp9,
	...
	setUP := func(usi string, first, ns16b, ns8b, ns4b, ns2b,
	...
	one_set_BOOL := func(ps16a, ps8a, ps4a, ps2a, ps1a, in4i, in2i, in1i bool,
		fault_C string) (bool, bool, bool, bool, bool, bool, bool, bool, string) {
	...
	peek := func(fp int, s nd, fault_A string, fault_C string,
		...
		pt16, pt8, pt4, pt2, pt1 := convertKeys(fp, s) // fp OR output
		...
		// ns2fp =======================================================
		dMfp0, dMfp1, dMfp2, dMfp3, dMfp4, dMfp5, dMfp6, dMfp7, dMfp8, dMfp9,
			dMfp10, dMfp11, dMfp12, dMfp13, dMfp14, dMfp15, dMfp16, dMfp17,
			dMfp18, dMfp19, dMfp20, dMfp21, dMfp22, dMfp23, dMfp24, dMfp25,
			dMfp26, dMfp27, dMfp28, dMfp29, dMfp30, dMfp31 :=
		...
		// fpz is an nd flag which indicates that fp == 0
		fpz = mfpz(fp)

		// lns is a flag which if True indicates that at least one ns-line or at least one output line is non-null
		//lns := or(pt1, or(pt2, or(pt4, or(pt8, or(pt16, or(and(fpz, out4_s), or(and(fpz, out2_s), and(fpz, out1_s))))))))
		...
		siMap = dispdM(dMfp0, lns, 0, allSAT, nd2str, siMap)
	...
	simulateSI := func(si string, fault_C string) SimResult {
		...
		// one_set_BOOL := func(ps16a, ps8a, ps4a, ps2a, ps1a, in4i, in2i, in1i bool,fault_C string)
		...
		fmt.Println("  fault_A:", originalFaultA, "fault_C:", fault_C)
	...
	// If I get this far, I have entered something and it's not xx1 - xx4
	// So it's either a not allowed input, or is 999
	fmt.Println("==== Starting main loop ====")
	// Modified main loop with separated phases
	var fault_A, fault_C string

	// Initialize --- THIS IS AN EQIVALENT OF FOO() IN THE ORIGINAL CODE
	resetAccumulatedSI()
	...
	for {
		...
		if fault_A == "" { // I want to retain the sequence and go to 999, xx1, . . .
			// and I don't want to display anoter peek() output
		...
		if fault_C == "" {
			...
			for {
				...
				fmt.Print("\nEnter selected S/I (example: s12i5) or control (999/xx1/xx2/xx3/xx4): ")
				fmt.Println("\n=== #3 ===")
				var input string
				fmt.Scanln(&input)
				...
				default:
					...
					if strings.HasPrefix(input, "s") && strings.Contains(input, "i") {
						...
						if n, _ := fmt.Sscanf(input, "s%di%d", &testS, &testI); n == 2 {
							...
							if testS >= 0 && testS <= 31 && testI >= 0 && testI <= 7 {
								...
								if mapping, exists := siMap[input]; exists {
									...
									fp = mapping.fp
									ns = mapping.ns
									fmt.Printf("Retrieved from siMap: %s -> fp=%d\n", input, fp)
						...
						fmt.Println("\n=== #16 ===")
						fmt.Println("Invalid input. Example S/I format (s12i5) or control codes (999/xx1/xx2/xx3/xx4)")
					}
