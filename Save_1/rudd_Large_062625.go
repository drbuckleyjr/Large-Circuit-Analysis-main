// rudd_Large_062625.go
// version 29 JUN 2025 @ 2000 hrs
// uses fault_A, fault_C
...
func main() {
	...
	// ---- type changing function string -> nd ----
	...
	// ---- function to tranlate an nd function into its constituent S/I's ----
	...
	peek := func(fp int, s nd, fault_A, fault_C string) (int, string) {
		...
		// ---- type changing function nd -> string ----
		...
		// ---- lns = at least one of pt1, pt2, pt4, pt8, pt16 ----
		...
		// Modify dispdM to accept nd2str as a parameter
		dispdM := func(dMfpx S, lns nd, fpn int, nd2strFunc func(nd) string) {
			...
			if a != 0 {
				for i := 0; i < a; i++ {
					...
					if !eq(and(lns, dMfpx[i].si), null) || eq(lns, null) {
						fmt.Println(allSAT(dMfpx[i].si), "  ", allSAT(dMfpx[i].out_4),
							allSAT(dMfpx[i].out_2), allSAT(dMfpx[i].out_1),
							"  fp =", fpn, "  ns =", nd2strFunc(dMfpx[i].ns))
					}
				}
			}
		}

		// These result in printing the <S/I, outputs, fp, ns> objects
		// for each fault pattern fp1, fp2, ..., fp31

		dispdM(dMfp1, lns, 1, nd2str)
		dispdM(dMfp2, lns, 2, nd2str)
		dispdM(dMfp3, lns, 3, nd2str)
		dispdM(dMfp4, lns, 4, nd2str)
		dispdM(dMfp5, lns, 5, nd2str)
		dispdM(dMfp6, lns, 6, nd2str)
		dispdM(dMfp7, lns, 7, nd2str)
		dispdM(dMfp8, lns, 8, nd2str)
		dispdM(dMfp9, lns, 9, nd2str)
		dispdM(dMfp10, lns, 10, nd2str)
		dispdM(dMfp11, lns, 11, nd2str)
		dispdM(dMfp12, lns, 12, nd2str)
		dispdM(dMfp13, lns, 13, nd2str)
		dispdM(dMfp14, lns, 14, nd2str)
		dispdM(dMfp15, lns, 15, nd2str)
		dispdM(dMfp16, lns, 16, nd2str)
		dispdM(dMfp17, lns, 17, nd2str)
		dispdM(dMfp18, lns, 18, nd2str)
		dispdM(dMfp19, lns, 19, nd2str)
		dispdM(dMfp20, lns, 20, nd2str)
		dispdM(dMfp21, lns, 21, nd2str)
		dispdM(dMfp22, lns, 22, nd2str)
		dispdM(dMfp23, lns, 23, nd2str)
		dispdM(dMfp24, lns, 24, nd2str)
		dispdM(dMfp25, lns, 25, nd2str)
		dispdM(dMfp26, lns, 26, nd2str)
		dispdM(dMfp27, lns, 27, nd2str)
		dispdM(dMfp28, lns, 28, nd2str)
		dispdM(dMfp29, lns, 29, nd2str)
		dispdM(dMfp30, lns, 30, nd2str)
		dispdM(dMfp31, lns, 31, nd2str)
	...
	foo := func() {
		...
		for true {
			...
			var fault_C string
			fmt.Scanln(&fault_C)
			// inner conntinuous loop peek
			first - time := true
			for true {
				if first - time {
					ufp, uns := peek(0, s0, fault_A, fault_C)
					ufph, unsh := ufp, uns
					first - time = false
				} else {
					// ---- peek() prints a list of objects
					//      of type g: <S/I, outputs, fp, ns>
					ufph, unsh = peek(ufph, unsh, fault_A, fault_C)
					// -----------------------------------
					// ---- and foo() asks the user to select an S/I ----
				}
				fmt.Println("========================================================")
				...
				fmt.Scanln(&usi)
				fmt.Println()
				// ---- using the selected S/I, we simulate a next time-frame

				//      simulation of the circuit, and report results ----
				// -----------------------------------------------------------
				...
				fmt.Println("========================================================")
				fmt.Println("simulation:  out4: ", out4b, ",  out2: ", out2b, ",  out1: ", out1b)
				fmt.Println("simulation next_state:", state)
				// fmt.Println("next_state:", state, "    fault_C:", fault_C)
				fmt.Println("========================================================")
