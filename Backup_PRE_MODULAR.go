...
func activatePropagateFaultA(ps16_i, ps8_i, ps4_i, ps2_i, ps1_i nd,
	...
	// DEBUG: Print outputs leaving activatePropagateFaultA
	fmt.Printf("=== EXITING activatePropagateFaultA ===\n")
...
func displayAvailableTransitions(fp int, s nd, fault_A string, _ string, allSAT func(nd, func(string) nd) []string, nd2str func(nd) string, siMap map[string]SIMapping) map[string]SIMapping {
	...
	out4_s, out2_s, out1_s, ns16_s, ns8_s, ns4_s, ns2_s, ns1_s :=
		activatePropagateFaultA(pt16, pt8, pt4, pt2, pt1, fault_A)
	// =============================================================
	...
	// Define all possible faults for Large circuit (excluding fault_A)
	allPossibleFaults := getAllPossibleFaults()

	// Helper function to count "Same As Fault_A" faults for a given S/I
	countSameAsFaultA := func(candidateSI string) int {
	...
	fmt.Printf("Analyzing fault interference for fault_A: %s\n", fault_A)
			...
			for i := 0; i < a; i++ {
				...
				if !isNull(and(lns, dMfpx[i].si)) || isNull(lns) {
					...
					if siStr != "" {
						...
						// Calculate fault interference count for this S/I choice
						sameAsFaultACount := countSameAsFaultA(siStr) // Enhanced display with outputs and fault interference count
	...
	for {
		...
		if fault_C == "" {
			...
			for {
				...
				fmt.Print("\nEnter selected S/I (example: s12i5) or control (999/xx0/xx1/xx2/xx3/xx4/debug/xxAdaptive/xxUndo): ")
				var input string
				fmt.Scanln(&input)
